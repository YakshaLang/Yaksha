...
namespace yaksha {
   ...
   */
  struct codefiles;
  struct def_class_visitor : stmt_visitor {
    ...
    bool has_enum(const std::string &prefixed_name);
    enum_stmt *get_enum(const std::string &prefixed_name);
...
private:
    ...
    std::unordered_map<std::string, enum_stmt *> enums_{};
    builtins *builtins_;
    codefiles *cf_;
    ...
    bool is_redefined(const std::string &name, token *tok);
  };
}// namespace yaksha
#endif
