...
expr *ast_pool::c_assign_expr(token* name, token* opr, expr* right, bool promoted) {
  auto o = new assign_expr(name, opr, right, promoted);
...
stmt *ast_pool::c_const_stmt(token* name, ykdatatype* data_type, expr* expression) {
...
foreach_stmt::foreach_stmt(token* for_keyword, token* name, ykdatatype* data_type, token* in_keyword, expr* expression, stmt* for_body)
    : for_keyword_(for_keyword), name_(name), data_type_(data_type), in_keyword_(in_keyword), expression_(expression), for_body_(for_body) {}
...
stmt *ast_pool::c_foreach_stmt(token* for_keyword, token* name, ykdatatype* data_type, token* in_keyword, expr* expression, stmt* for_body) {
  auto o = new foreach_stmt(for_keyword, name, data_type, in_keyword, expression, for_body);
  cleanup_stmt_.push_back(o);
  return o;
}
...
nativeconst_stmt::nativeconst_stmt(token* name, ykdatatype* data_type, token* ccode_keyword, token* code_str)
    : name_(name), data_type_(data_type), ccode_keyword_(ccode_keyword), code_str_(code_str) {}
...
stmt *ast_pool::c_nativeconst_stmt(token* name, ykdatatype* data_type, token* ccode_keyword, token* code_str) {
