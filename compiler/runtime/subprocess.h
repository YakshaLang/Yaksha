...
int subprocess_create_ex(const char *const commandLine[], int options,
  ...
  if (subprocess_option_inherit_environment !=
    ...
    if (SUBPROCESS_NULL == environment) {
      ...
      for (i = 0; environment[i]; i++) {
        for (j = 0; '\0' != environment[i][j]; j++) { len++; }
  ...
  if (!SetHandleInformation(rd, handleFlagInherit, 0)) { return -1; }
  fd = _open_osfhandle(SUBPROCESS_PTR_CAST(subprocess_intptr_t, rd), 0);
  if (-1 != fd) {
    out_process->stdout_file = _fdopen(fd, "rb");
    if (SUBPROCESS_NULL == out_process->stdout_file) { return -1; }
  }
  ...
  if (subprocess_option_combined_stdout_stderr ==
    ...
    if (-1 != fd) {
      ...
      if (SUBPROCESS_NULL == out_process->stderr_file) { return -1; }
    }
  ...
  if (0 != pipe(stdinfd)) { return -1; }
  if (0 != pipe(stdoutfd)) { return -1; }
  if (subprocess_option_combined_stdout_stderr !=
      (options & subprocess_option_combined_stdout_stderr)) {
    if (0 != pipe(stderrfd)) { return -1; }
  }
  child = fork();
  if (-1 == child) { return -1; }
  if (0 == child) {
...
int subprocess_join(struct subprocess_s *const process,
  ...
  if (process->child) {
    if (process->child != waitpid(process->child, &status, 0)) { return -1; }
    process->child = 0;
    if (WIFEXITED(status)) {
  ...
  if (out_return_code) { *out_return_code = process->return_status; }
  return 0;
#endif
}
int subprocess_destroy(struct subprocess_s *const process) {
  ...
  if (process->hProcess) {
    ...
    if (process->hStdInput) { CloseHandle(process->hStdInput); }
    if (process->hEventOutput) { CloseHandle(process->hEventOutput); }
    if (process->hEventError) { CloseHandle(process->hEventError); }
  }
#endif
  return 0;
}
...
int subprocess_alive(struct subprocess_s *const process) {
...
#else
  {
    ...
    if (!is_alive) {
      ...
      // the child now.
      process->child = 0;
      if (subprocess_join(process, SUBPROCESS_NULL)) { return -1; }
    }
  }
#endif
  if (!is_alive) { process->alive = 0; }
  return is_alive;
}
#if defined(__cplusplus)
}// extern "C"
