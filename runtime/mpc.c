...
static void mpc_input_unmark(mpc_input_t *i) {
  ...
  if (i->type == MPC_INPUT_PIPE && i->marks_num == 0) {
    for (j = strlen(i->buffer) - 1; j >= 0; j--) ungetc(i->buffer[j], i->file);
    free(i->buffer);
    i->buffer = NULL;
  }
}
...
mpc_parser_t *mpc_and(int n, mpc_fold_t f, ...) {
  ...
  for (i = 0; i < (n - 1); i++) { p->data.and.dxs[i] = va_arg(va, mpc_dtor_t); }
  va_end(va);
  return p;
}
/*
...
mpc_ast_t *mpc_ast_get_child_lb(mpc_ast_t *ast, const char *tag, int lb) {
  int i;
  for (i = lb; i < ast->children_num; i++) {
    if (strcmp(ast->children[i]->tag, tag) == 0) { return ast->children[i]; }
  }
  return NULL;
}
